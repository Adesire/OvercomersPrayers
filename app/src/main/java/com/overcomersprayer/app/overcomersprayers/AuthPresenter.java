...
import static com.overcomersprayer.app.overcomersprayers.activities.LoginActivity.RC_SIGN_IN;
        ...
        mAuth.signInWithEmailAndPassword(email, password).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                if(mAuth.getCurrentUser().isEmailVerified()){
                    ...
                    Toast.makeText(loginActivity, "Email address not verified, kindly verify", Toast.LENGTH_SHORT).show();
                    mAuth.signOut();
                    ...
                    if (task12.isSuccessful()) {
                        ...
                        reference.child(Users.getTableName()).child(firebaseUser.getUid()).setValue(user).addOnCompleteListener(task1 -> {
                            if (task1.isSuccessful()) {
                                sendVerification();//mAuthListener.onAuthSuccess();
                            } else {
    ...
    public void sendVerification(){
        ...
        if (user != null) {
            user.sendEmailVerification()
                    .addOnCompleteListener(loginActivity, (OnCompleteListener) task -> {
                        if (task.isSuccessful()) {
                            Log.e(TAG,"Verification email sent to " + user.getEmail());
                            Toast.makeText(loginActivity, "Registration success kindly verify your account via email", Toast.LENGTH_SHORT).show();
                            mAuth.signOut();
                            loginActivity.recreate();
                        } else {
